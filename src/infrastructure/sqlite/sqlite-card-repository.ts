...
class SqliteCardRepository implements CardRepository {
  ...
  async getByNote(noteId: string): Promise<readonly Card[]> {
    return (
      await this.db.select().from(cards).where(eq(cards.noteId, noteId)).orderBy(asc(cards.ordinal))
    ).map(toDomainCard);
  }
  ...
  async getDue(deckId: string, at: number, limit: number): Promise<readonly Card[]> {
    ...
    const rows = await this.db
      .select()
      .from(cards)
      .where(and(eq(cards.deckId, deckId), eq(cards.isSuspended, false), lte(cards.dueAt, at)))
      .orderBy(asc(cards.dueAt), asc(cards.ordinal))
      .limit(limit);
